...
Prove periodic tasks meet their deadlines under rate-monotonic and EDF. `rt_scheduling.py`:

```
$ python examples/rt_scheduling_demo.py examples/output

  RM Liu-Layland bound (3 tasks) 0.78; EDF exact iff U <= 1
  set U=0.97: RM fails, EDF schedules (dynamic priorities use the CPU fully)
```

Rate-monotonic (fixed priority by rate) is schedulable if U <= n(2^(1/n)-1); EDF (nearest deadline
first) is exactly schedulable iff U <= 1. Both are checked against a hyperperiod simulation (the LCM of
...
Amplitude and frequency demodulation via the analytic signal. `hilbert.py`:

```
$ python examples/hilbert_demo.py examples/output

  AM envelope recovered to 0.0000; chirp instantaneous freq tracks 12.5 -> 45 Hz
```

The analytic signal z = x + i H[x] = A e^{i phi} gives the envelope A(t) and the phase phi(t) whose
derivative is the instantaneous frequency, built by zeroing negative frequencies in the DFT and
...
Biologically sensible sequence alignment where one long gap beats many short ones. `affine_alignment.py`:

```
$ python examples/affine_alignment_demo.py examples/output

  inserted block aligns as ONE contiguous gap (open -4, extend -1)
  affine with open=0 reproduces linear Needleman-Wunsch exactly
```

Gotoh's algorithm runs three DP matrices (match, gap-in-x, gap-in-y) so gap opening costs more than
