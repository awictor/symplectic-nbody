...
def main():
    ...
    sections = [
        ...
        section(
            ...
            "WAVE FUNCTION COLLAPSE fills a grid with tiles so that every adjacency obeys local rules, "
            "producing outputs that look hand-authored -- coherent maps, textures, levels -- rather "
            "than random. The name is a quantum metaphor: each cell starts in a SUPERPOSITION of "
            "every tile, and the algorithm repeatedly OBSERVES (collapses) the most-constrained cell "
            "to one tile, then PROPAGATES that choice, deleting any option a neighbour can no longer "
            "support -- the discrete constraint-satisfaction loop of arc consistency. Observation "
            "picks the lowest-ENTROPY cell (fewest remaining options) and collapses it by weight; "
            "propagation runs a worklist, shrinking neighbour option sets until the grid is "
            "arc-consistent, and if a cell's options ever hit zero the attempt is a CONTRADICTION and "
            "restarts. The adjacency rules -- which tiles may sit beside which, per direction -- are "
            "the whole specification, turning a tiny rule set into endless consistent worlds. This "
            "module implements tiled WFC with per-direction rules, weighted collapse, "
            "lowest-entropy observation, full propagation, contradiction detection with restart, and "
            "a seeded RNG, verified that every generated grid strictly satisfies the rules (no "
            "forbidden neighbour anywhere, across 20 seeds), that a seed reproduces its grid, that an "
            "over-constrained rule set is reported unsatisfiable rather than producing garbage, and "
            "that a rule set forcing a unique tiling produces exactly it (a perfect checkerboard).",
            '<div class="grid">'
            + svg_card(out("wave_function_collapse.svg"), "a coastline map generated by WFC: green land, sand coast, blue sea, where land and sea never touch because the coast tile is the only legal bridge -- every adjacency obeys the rules")
            + f'<div class="card">{pre(wfc_txt)}</div>'
            + '</div>'),
        section(
            "The Hungarian algorithm: optimal assignment in O(n^3)",
            "Given n workers, n jobs, and a COST for each pairing, which one-to-one assignment "
            "minimizes the total? This ASSIGNMENT PROBLEM appears everywhere -- taxis to riders, "
            "tasks to machines, tracked objects to detections between video frames. Brute force tries "
            "all n! permutations; the HUNGARIAN ALGORITHM (Kuhn-Munkres, 1955) finds the exact "
            "optimum in O(n^3), one of the first polynomial algorithms for a combinatorial problem. "
            "It rests on an invariant: subtracting a constant from a full row or column does not "
            "change WHICH assignment is optimal, only shifts every total equally. So the algorithm "
            "reduces rows and columns to expose zeros, selects n INDEPENDENT zeros (one per row and "
            "column -- a valid assignment), and when fewer than n exist, covers all zeros with a "
            "minimum set of lines (Konig's theorem) and subtracts the smallest uncovered value to "
            "create new zeros without breaking the old, until n independent zeros -- the optimum -- "
            "appear. This module implements the O(n^3) potential/augmenting-path form for "
            "rectangular matrices (padded to square), for both minimization and maximization, "
            "verified against brute-force search over all permutations (matching the exact optimum "
            "over 60 random square matrices and up to n=8), the row/column reduction invariant, "
            "identity and hand-worked matrices, and rectangular padding.",
            '<div class="grid">'
